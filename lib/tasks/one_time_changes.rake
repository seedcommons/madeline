namespace :one_time_changes do
  ...
  task update_loan_statuses_oct_22: :environment do
    ...
    dormant.update(value: 'possible', position: 40)
    ...
    statuses_to_update.each do |status|
      status =  loan_status.options.find_by(value: status)
      status.update(label_translations: {
        en: I18n.t("database.option_sets.loan_status.#{status}", locale: 'en'),
        es: I18n.t("database.option_sets.loan_status.#{status}", locale: 'es')
        })
    end
