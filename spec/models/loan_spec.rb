...
describe Loan, type: :model do
  ...
  context 'source_of_capital' do
    ...
    it 'defaults to shared' do
      expect(create(:loan).source_of_capital).to eq "shared"
    end

    it 'can be a value in SOURCE_OF_CAPITAL_OPTIONS' do
      expect {
    ...
    describe 'calculated fields: .sum_of_repayments, .sum_of_disbursements, .change_in_interest, .change_in_principal' do
      ...
      context "multiple transactions" do
        ...
        it "limit by date, inclusive" do
          ...
          expect(loan.total_interest_repaid(start_date: Date.parse('2018-01-01'), end_date: Date.parse('2020-01-01'))).to eq(1.2)
          expect(loan.total_interest_repaid(start_date: Date.parse('2019-01-04'), end_date: Date.parse('2020-01-01'))).to eq(1.0)
          expect(loan.total_interest_repaid(start_date: Date.parse('2019-01-05'), end_date: Date.parse('2020-01-01'))).to eq(0.6)
        end
