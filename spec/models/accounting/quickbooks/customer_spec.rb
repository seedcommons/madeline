...
RSpec.describe Accounting::Quickbooks::Customer, type: :model do
  ...
  #   end
  #
  #   context 'when qb customer does exist in Madeline' do
  #     let(:organization) { create(:organization, qb_id: qb_customer_id) }
  #
  #     it 'does not create a customer' do
  #       expect(service).not_to receive(:create)
  ...
  #
  #
  # context 'when there is a duplicate in quickbooks' do
  #   let(:organization) { create(:organization, name: "duplicate org") }
  #   before do
  #     allow(service).to receive(:create).once.and_raise(::Quickbooks::IntuitRequestException, 'Duplicate Name Exists Error')
  #     allow(service).to receive(:create).once.and_return(qb_new_customer)
  #     #allow(service).to receive(:find_by).and_return(:qb_query_result)
  #   end
  #
  #   context 'duplicate is a customer' do
  #     let(:qb_query_result) { double(results: [{id: qb_customer_id}])}
  #
  #     it "works" do
  #       reference = subject.reference
  #       expect(reference.type).to eq 'Customer'
  ...
  #   end
  #
  #   context 'duplicate is not a customer' do
  #     let(:qb_query_result) { double(results: [])}
  #
  #     it "works" do
  #       reference = subject.reference
  #       expect(reference.type).to eq 'Customer'
