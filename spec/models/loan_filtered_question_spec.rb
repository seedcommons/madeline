...
RSpec.describe LoanFilteredQuestion, type: :model do
  ...
  describe '#required?' do
    it 'not required by default' do
      expect(described_class.new(q1, loan: loan1, user: user)).not_to be_required
    end

    it 'required when override true and assocation present' do
      expect(described_class.new(q3, loan: loan1, user: user)).to be_required
    end

    it 'not required when override true and assocation not present' do
      expect(described_class.new(q3, loan: loan2, user: user)).not_to be_required
    end

    it 'required when inherited and parent association present' do
      expect(described_class.new(q31, loan: loan1, user: user)).to be_required
    end

    it 'not required when inherited and parent association not present' do
      expect(described_class.new(q31, loan: loan2, user: user)).not_to be_required
    end

    it 'not required when override true for child and not present at child level' do
      expect(described_class.new(q332, loan: loan1, user: user)).not_to be_required
    end

    it 'required when override true for child and present at child level' do
      expect(described_class.new(q332, loan: loan2, user: user)).to be_required
    end

    it 'required when override true and association present for both types' do
      expect(described_class.new(q2, loan: loan2, user: user)).to be_required
    end

    it 'not required when override true for child and no associations present' do
      expect(described_class.new(q381, loan: loan1, user: user)).not_to be_required
    end

    it 'not required on child when override false even when association is present' do
      expect(described_class.new(q4, loan: loan1, user: user)).not_to be_required
    end
  end
end
