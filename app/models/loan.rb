...
class Loan < Project
  ...
  def loan_media(limit=100, images_only=false)
    get_media(limit: limit, images_only: images_only)
  end
  ...
  def featured_pictures(limit=1)
    ...
    # then loan pics
    pics += get_media(limit: limit - pics.count, images_only: true)
    return pics unless limit > pics.count
    # then log pics
  ...
  def thumb_url
    if featured_pictures.any?
      featured_pictures.first.item.url(:thumb)
    else
