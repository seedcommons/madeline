...
module Accounting
  class InterestCalculator
    ...
    def record_and_rollback_changes(txn)
      ::Accounting::LoanIssue.create(loan: loan, accounting_transaction: txn, message: :attempted_change_before_closed_books_date, custom_data: {cbd: @closed_books_date, txn_date: txn.txn_date}, level: :warning)
    ...
    def add_int_tx?(txs, prev_tx, loan)
      if txs.nil? # this is an end of month day with no txns
        if prev_tx.txn_date > @closed_books_date
          ...
          ::Accounting::LoanIssue.create(loan: prev_tx.project, accounting_transaction: prev_tx, message: :no_end_of_month_int_txn_before_closed_books_date, custom_data: {cbd: @closed_books_date, txn_date: prev_tx.txn_date}, level: :warning)
        end
      elsif prev_tx && prev_tx.principal_balance > 0 && txs.none?(&:interest?)
        if prev_tx.txn_date > @closed_books_date
          return true
        else
          ::Accounting::LoanIssue.create(loan: loan, accounting_transaction: prev_tx, message: :attempted_new_int_txn_before_closed_books_date, custom_data: {cbd: @closed_books_date, txn_date: prev_tx.txn_date}, level: :warning)
        end
      end
      false
    end
