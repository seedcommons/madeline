    ...
    class LoanIssuesController < Admin::AdminController
      def index
        ...
        if params[:loan_id]
          ...
          @plts = ::Accounting::LoanIssue.where(accounting_transaction_id: txn_ids).group_by(&:txn_id)
        else
          @plts = ::Accounting::LoanIssue.all.sort_by(&:project_id).group_by(&:project_id)
          render :index_by_loan
        end
      end

      def show
        @plt = ::Accounting::LoanIssue.find(params[:id])
        authorize @plt
      end
    end
  end
