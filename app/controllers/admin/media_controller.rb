class Admin::MediaController < Admin::AdminController
  ...
  def index
    @media = policy_scope(Media.where(media_attachable_type: SUPPORTED_TYPES).order(created_at: :desc)).page(params[:page]).per(12)
    authorize @media
  end
  ...
  def media_params
    params.require(:media).permit(*([:item, :kind_value, :featured] + translation_params(:caption, :description)))
  end
end
